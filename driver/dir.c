...
//
int gfs_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
           loff_t pos    = filp->f_pos;             // Current position in dir.
  struct   inode *inode  = filp->f_dentry->d_inode; // Dir's inode.
  ...
  if ((char *)de <= limit) {
    over = filldir(
      dirent, de->name, de->name_length, le32_to_cpu(de->next_offset),
//      dirent, de->name, de->name_length, (n << PAGE_CACHE_SHIFT) | offset,
      le32_to_cpu(de->inode), DT_UNKNOWN);

    // If there are no more entires, set the file position to EOF.
    if (de->next_offset == 0) filp->f_pos = inode->i_size;
    else {
      filp->f_pos = le32_to_cpu(de->next_offset);
    }
  }
  gfs_put_page(page);
