%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% FILE   : doc-Implementation.tex
% SUBJECT: Implementation details for GenericFS
...
it\footnote{Keep in mind that the documentation in \filename{Documentation}, like all Linux
...
systems that have already been implemented. For example the directory \filename{fs/ext2}
...
returns the size of the partition in 512 byte sectors. \code{BLOCKSIZE} is 4 Kbytes in our case.
...
allocate the \code{gfs_inode_info} structures efficiently. Thus the driver creates a kernel SLAB
...
\filename{/proc/slabinfo}). Figure \ref{fig:init-slab} shows how a SLAB cache is created for the
inode structures.
...
\code{kmem_cache_create}. The SLAB cache calls this function for each object it allocates. In
...
\code{open} you will need to at least provide a place holder method so that you can install a
